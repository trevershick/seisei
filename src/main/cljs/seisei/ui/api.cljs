...
(defn- load-my-account-or-home-handler
  [acct]
  (debug "api/load-my-account-or-home GET /my/account, response=" acct)
  (if (:logged-in acct)
    (d/action :my-account-received acct)
    (.open js/window "/#")))

(defn load-my-account-or-home []
  (debug "api/load-my-account-or-home")
  (GET "/my/account"
       {:keywords?        true
        :response-format  :json
        :handler          load-my-account-or-home-handler}))
